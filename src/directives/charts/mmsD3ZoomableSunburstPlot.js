'use strict';
 angular.module('mms.directives')
    .directive('mmsD3ZoomableSunburstPlot', ['TableService', '$window', mmsD3ZoomableSunburstPlot]);
function mmsD3ZoomableSunburstPlot(TableService,  $window) {
  ...
  var mmsChartLink = function(scope, element, attrs, mmsViewCtrl) {
    ...
    var defaultPlotConfig = {width : 900, marginTop:0, marginRight:0, marginBottom:25, marginLeft:60};
    ...
    function format(value){ return formats(value);}
	var formats = d3.format(",d");
	var radius = width / 6;
	
	function labelTransform(d) {
		var x = (d.x0 + d.x1) / 2 * 180 / Math.PI;
		var y = (d.y0 + d.y1) / 2 * radius;
		var rotateValue = x < 180 ? 0 : 180; 
		return "rotate(" + (x-90) + ") translate(" + y + ",0) rotate(" + rotateValue + ")";
	}
	var arcs = d3.arc()
	...
	function arc( value){ return arcs(value);}	
	...
	//add name and empty children
	function addChild(children, celldata, value){
		var key = "name";
		var contain= false;
		for ( var j = 0; j < children.length; j++ ){
			if ( children[j][key] === celldata){
				contain = true;
		...
		if (!contain) {
			if (value == null)
				children.push({name: celldata, children: []});
			else
				children.push({name: celldata, value: value});
			return children[children.length-1].children; //newly added children
		}
	}
    scope.render = function() {
		console.log(scope.plot);
      TableService.readvalues1(scope.plot, projectId, refId, commitId, true)
       .then( function(value){
        ...
        scope.tablebody.c3_data.forEach( function (row){
            ...
            dataseries.push( {row: row[0], values: values});
        });
		
		var myModel = {name: scope.plot.title, children: []};
		scope.tablebody.c3_data.forEach( function (row, i){
			var key = "name";
			var temp = myModel.children;
			var parent;
			for (var j = 0; j < row.length - 1; j++ ){
				if (j == row.length - 2)
					parent = addChild(temp, row[j], row[j+1]);
				else	
					parent = addChild(temp, row[j], null);
				temp = parent;
			}
	...
	var ZoomableSunburst = {
		draw: function(id, root, colors) {
	
		var svg = divchart.append("svg:svg")
				.attr("class", "zc" + scope.$id)
		  ...
		  .attr("transform", "translate(" + width/2 + "," + width/2 + ")");
			  ...
			  var path = g.append("g")
				...
				.enter().append("path")
				  ...
				  .attr("fill-opacity", function(d){return arcVisible(d.current) ? (d.children ? 0.6 : 0.4) : 0;})
			  ...
			  path.append("title")
				  ...
				  //.text(function(d){d.ancestors().map(function(d){return d.data.name}).reverse().join("/") + "\n" + format(d.value);});
			...
			function clicked(p, i) { //event is not passed
			   ...
			   path.transition(t)
				  ...
				  .filter(function(d) {
					...
					.attr("fill-opacity", function(d){ return arcVisible(d.target) ? (d.children ? 0.6 : 0.4) : 0;})
				...
				function color(name) { return colors(name);}
			}//end of draw

		};//endof ZoomableSunburst
